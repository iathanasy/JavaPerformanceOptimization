 ...
 * @see LinkedHashMap 继承HashMap 在内部存放了一个链表，用以存放元素的顺序. 提供两种顺序：1.元素插入时顺序
 *      2.元素最近访问的顺序 accessOrder = true
 * @see TreeMap 实现SortedMap接口 可以对元素进行排序 性能略低于HashMap. 内部基于红黑树实现
 * @see Hashtable 实现Map接口 与HashMap差异不大。
 *      Hashtable做了同步，线程安全、不允许key和value出现null值、而HashMap可以。 在内部算法上
 *      key的hash算法和hash值到内存索引的映射算法不同
 * @author cd
...
public class MapTest {
	...
	public static void main(String[] args) {
		 ...
		 * ); for(Iterator<String> it = linked.keySet().iterator();
		 * it.hasNext();){ String key = it.next();
		 *//**
		 * new LinkedHashMap<String, Object>(16,0.75f,true); 指定设置true
		 * 按照访问时间排序 map.get会去修改集合中的结构，所以会抛出异常
		 * java.util.ConcurrentModificationException 不要在迭代器模式中修改被迭代的集合 会抛出上面的异常
		 * 这个特性适用于所有集合类 包括HashMap、Vector、ArrayList
		 */
		/*
