...
public class ListTest {
	...
	public static void main(String[] args) {
		/**
		 * arr: 101 linked: 6 add() ArrayList 数组实现 -> 要进行数组重组 越往前插入重组时间越长
		...
		System.out.println(" foreach linked: "
				+ (System.currentTimeMillis() - begin));
		...
		System.out.println(" for linked: "
				+ (System.currentTimeMillis() - begin));

		// ArrayList删除
		begin = System.currentTimeMillis();
		/*
		 * while(arr.size() > 0){ arr.remove(0);//头部删除 arr rm: 180 }
		 */
		/*
		 * while(arr.size() > 0){ arr.remove(arr.size() >> 1);//中间删除 arr rm: 98
		 * }
		 */
		while (arr.size() > 0) {
		...
		begin = System.currentTimeMillis();
		/*
		 * while(arr.size() > 0){ arr.remove(0);//头部删除 linked rm: 0 }
		 */
		/*
		 * while(arr.size() > 0){ arr.remove(arr.size() >> 1);//中间删除 linked rm:
		 * 1 }
		 */
		while (arr.size() > 0) {
		...
		System.out
				.println("linked rm: " + (System.currentTimeMillis() - begin));

		/***
		 * ArrayList随机访问是很快的 增删改会导致数组 重组 LinkedList 基于链表实现 随机访问性能很差 增删改很快
		 * 
		 * 查询的数据 LinkedList for循环是很慢的 性能很差 foreach arr: 6 Iterator arr: 4 for
		 * arr: 2 foreach linked: 5 Iterator linked: 2 for linked: 3670
		 * 
		 * 
		 */
	}
