...
public class HttpClientDemo {
	...
	/**
	 * post提交多类型参数 针对<form enctype="multipart/form-data">表单
	 */
	 ...
	 * HttpRequestRetryHandler retryHandler = (exception, executionCount,
	 * context) -> { if (executionCount >= 3) { return false; } if (exception
	 * instanceof InterruptedIOException) { return false; } if (exception
	 * instanceof UnknownHostException) { return false; } if (exception
	 * instanceof ConnectTimeoutException) { return false; } if (exception
	 * instanceof SSLException) { return false; } HttpClientContext
	 * clientContext = HttpClientContext.adapt(context); HttpRequest request =
	 * clientContext.getRequest(); boolean idempotent = !(request instanceof
	 * HttpEntityEnclosingRequest); if (idempotent) { return true; } return
	 * false; }; CloseableHttpClient httpClient = HttpClients.custom()
	 * .setRetryHandler(retryHandler) .build(); httpClient.execute(new
	...
	/**
	 * cookie管理方式一 初始化HttpClient时，传入一个自己CookieStore对象
	 */
	...
	/**
	 * cookie管理方式二 每次执行请求的时候传入自己的HttpContext对象
	 */
	...
	public static void httpProxy() throws IOException {
		CloseableHttpClient httpClient = HttpClients.custom()
				// 设置代理
				.setRoutePlanner(
						new DefaultProxyRoutePlanner(new HttpHost("localhost",
								8888))).build();
		CloseableHttpResponse response = httpClient.execute(new HttpGet(
	 ...
	 */
	/*
	 * public static void SSLConfig() throws Exception { //默认信任 SSLContext
	 ...
	 * RegistryBuilder.<ConnectionSocketFactory>create() .register("http", new
	 * PlainConnectionSocketFactory()) .register("https", new
	 * SSLConnectionSocketFactory(sslContext)) .build(); CloseableHttpClient
	 * httpClient = HttpClients.custom() .setConnectionManager(new
	 * PoolingHttpClientConnectionManager(socketFactoryRegistry)) .build();
	 * httpClient.execute(new HttpGet("https://www.baidu.com")); }
	 */

	static class SocketProxyPlainConnectionSocketFactory extends
	...
	static class SocketProxySSLConnectionSocketFactory extends
	 ...
	 */
	/*
	 * public static void socketProxy() throws Exception { //默认信任 SSLContext
	 * sslContext = SSLContexts.custom()
	 * .loadTrustMaterial(KeyStore.getInstance(KeyStore.getDefaultType()) ,
	 * (chain, authType) -> true).build(); Registry<ConnectionSocketFactory>
	 ...
	 * httpClientContext.setAttribute("socks.address", new
	 * InetSocketAddress("127.0.0.1", 1086)); CloseableHttpResponse response =
	 * httpClient.execute(new HttpGet("https://httpbin.org/ip"),
	 * httpClientContext);
	 * System.out.println(EntityUtils.toString(response.getEntity())); }
	 */
	/**
	...
	public static void downloadFile() throws Exception {
		...
		Files.copy(is, new File("temp.png").toPath(),
